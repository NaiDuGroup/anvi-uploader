...
import { DEFAULT_PHOTO_SETTINGS } from "@/lib/notebook/templates";
import {
  BG_COLOR_OPTIONS,
  FONT_OPTIONS,
  TEXT_COLOR_OPTIONS,
...
// Re-exported so existing call sites (NotebookCanvasPreview, callers that read
// the default font) keep working without a churn-only refactor.
...
interface NotebookEditorProps {
  ...
  /**
   * Optional surface colour of the physical notebook (cover hex). When
   * provided the background and text-colour palettes hide swatches that are
   * visually indistinguishable from the cover, so e.g. a red notebook never
   * offers a red background. Pass `null`/`undefined` for "Other" SKUs to
...
export function NotebookEditor({
  ...
  const textPaletteFiltered =
    visibleTextColors.length < TEXT_COLOR_OPTIONS.length;
  const bgPaletteFiltered = visibleBgColors.length < BG_COLOR_OPTIONS.length;

  // Snap to a sensible neighbour when the user picks a SKU that no longer
  // offers the currently selected colour (e.g. switched to a red notebook
  // while #DC2626 was the active text colour).
  useEffect(() => {
    if (
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-2">
        <h3 className="text-sm font-semibold text-gray-700">{t.notebook.uploadPhotos}</h3>
        ...
        {maxPhotos > 1 && (
          <p className="text-xs text-gray-400">{t.notebook.maxPhotos(maxPhotos)}</p>
        )}
      </div>
