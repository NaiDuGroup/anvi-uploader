...
import {
  X, Plus, Upload, FileText, Trash2, Loader2, Download,
} from "lucide-react";
...
type EditFileEntry = ExistingFile | NewFile;
...
interface OrderItemData {
  ...
  notes: string | null;
  files: { id: string; fileName: string; fileUrl: string; pageCount: number | null }[];
}
...
interface EditableOrder {
  ...
  totalPrice: number | null;
  items: OrderItemData[];
  files: { id: string; fileName: string; fileUrl: string; copies: number; color: string; paperType: string | null; pageCount: number | null }[];
}
...
export default function EditOrderModal({
  ...
  // Build initial items from existing order
  const buildInitialItems = useCallback((): EditItemForm[] => {
    if (order.items.length > 0) {
      return order.items.map((item) => ({
        ...
        activeSurchargeKeys: ((item as unknown as Record<string, unknown>).activeSurchargeKeys as string[] | undefined) ?? [],
        attributes: item.attributes ?? {},
        notes: item.notes ?? "",
        files: item.files.map((f): ExistingFile => ({
          type: "existing",
          id: f.id,
          fileName: f.fileName,
          fileUrl: f.fileUrl,
    ...
    if (order.files.length > 0) {
      return [{
        ...
        attributes: {},
        notes: "",
        files: order.files.map((f): ExistingFile => ({
          type: "existing",
          id: f.id,
          fileName: f.fileName,
          fileUrl: f.fileUrl,
  ...
  const [items, setItems] = useState<EditItemForm[]>(buildInitialItems);
  ...
  const updateItem = (index: number, patch: Partial<EditItemForm>) => {
    setItems((prev) => {
      const next = [...prev];
  ...
  const removeItem = (index: number) => {
    setItems((prev) => {
      ...
      entry.files.forEach((f) => {
        if (f.type === "new" && f.previewUrl?.startsWith("blob:")) {
  ...
  const addFiles = useCallback(async (itemIndex: number, newFiles: FileList | null) => {
    if (!newFiles) return;
    const entries: NewFile[] = await Promise.all(
      Array.from(newFiles).map(async (file) => {
        ...
        return { type: "new" as const, file, pageCount, previewUrl };
      }),
    );
    setItems((prev) => {
      const next = [...prev];
  ...
  const removeFile = (itemIndex: number, fileIndex: number) => {
    setItems((prev) => {
      ...
      if (entry?.type === "new" && entry.previewUrl?.startsWith("blob:")) {
          ...
          const fileData = await Promise.all(
            item.files.map(async (entry) => {
              if (entry.type === "existing") {
                return { existingFileId: entry.id };
              }
              // Upload new file
  ...
  const canSave = items.length > 0 && phone.length >= 8 && hasAllFiles && !saving;
...
function emptyItem(): EditItemForm {
  return {
    categoryId: "",
    productId: "",
    customerProvided: false,
...
function EditItemCard({
  ...
  locale: string;
  t: ReturnType<typeof useLanguageStore.getState>["t"];
  onUpdate: (patch: Partial<EditItemForm>) => void;
          ...
          <div className="border border-gray-100 rounded-lg divide-y divide-gray-50 mt-2">
            {item.files.map((entry, fileIdx) => (
              <div key={fileIdx} className="flex items-center gap-2 px-3 py-2">
                {entry.type === "new" && entry.previewUrl ? (
                  <img
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-gray-900 truncate">{entry.type === "existing" ? entry.fileName : entry.file.name}</p>
                  <p className="text-[10px] text-gray-400">
                    {entry.type === "new"
                      ? `${(entry.file.size / 1024).toFixed(1)} KB`
                      : null}
                    {entry.pageCount ? ` · ${entry.pageCount} p.` : null}
                    {entry.type === "existing" && (
