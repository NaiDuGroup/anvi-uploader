...
function drawPhotoIntoSlot(
  ...
  // existing 4 templates (no save/restore, no extra path).
  ...
  // we paint a filled mask shape with shadow set, then erase the fill leaving
  // only the dropped shadow underneath the image.
  if (hasShadow) {
    ctx.save();
...
/**
 * Image-into-rect helper extracted from the legacy renderer so both the
 * fast path and the decorated path share identical cover/contain logic.
 */
